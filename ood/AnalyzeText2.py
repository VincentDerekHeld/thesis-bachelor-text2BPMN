...
from ood.LLM_bool import decide_if_end_of_process
...
def determine_finish_activity(activity: Activity):
    if Constant.filter_finish_activities:
        if "finish the process" in str(activity.process.action) or "finish the repair" in str(activity.process.action):
