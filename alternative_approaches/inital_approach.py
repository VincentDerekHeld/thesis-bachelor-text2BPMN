...
def determine_condition(sent_root):  # sent_root = assamble #TODO Active vs. passiv
    active = True
...
def extract_from_active_sentence(sent_root, conditonal=False) -> []:
    ...
    actions.append(tokens_to_string(temp_action_tokens))  # TODO: delete
    ...
    for token in sent_root.children:
        ...
        if token.dep_ == "nsubj":
            actors.append(tokens_to_string(determinate_full_name(token)))  # TODO
        ...
        # TODO: FIlter prep of time (meanwhile ex.10)
        if token.dep_ == "prep":
...
def extract_from_passive_sentence(sent_root, conditional=False):
    ...
    for token in sent_root.children:
        ...
        if token.dep_ == "nsubj":  # TODO: above
...
def extract_from_sentence(sent_root):
    ...
    if active:  # TODO: Check if active or passive
...
def check_verb_for_advl(token):
    for sub_token in token.children:
        if sub_token.dep_ == "advcl":  # Verb
            ...
            """
            # TODO: Ex.7 sub_token = "is", but also include acomp "availble"
            # TODO: Untersuche neuen Satz mit active oder passive label
...
def check_verb_for_conditions(token):  # TODO: needs to be used
...
def start_process_analysis():
    ...
    for structure in tasks:
        # TODO: filter tasks that are irrelevant
        print("*" * 20)
...
def determine_end_activities():  # TODO:
    pass
