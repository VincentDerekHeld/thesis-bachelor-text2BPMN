...
def create_bpmn_model(structure_list: [Structure], actor_list: list, title: str, save_path: str, text_description: str,
    ...
    if LLM_syntax_improval:
        try:
            render_bpmn_model(improve_taks_labeles_LLM(input_syntax_rule_based, text_description), save_path)
...
def create_bpmn_description(structure_list: [Structure], actor_list: list, title: str,
    ...
    for structure in structure_list:
        if less_end_gateways2: determine_if_end_gateways_are_needed(structure)
        ...
        if isinstance(structure, Activity):
            print(f"Case 113: Activity: {structure}, activity.is_end_activity: {structure.is_end_activity}")
            ...
            # if less_end_gateways2: structure_contains_end_activity = False
            if less_end_gateways == False: end_gateway = "gateway_" + str(
                structure.id) + "_end"  # TODO:2023-12-09:  here End Gateway # end_gateway_1_end
            if less_end_gateways: need_end_gateway = True
            ...
            for branch in structure.branches:
                ...
                if less_end_gateways == False: need_end_gateway = True  # TODO:2023-12-09:  here End Gateway
                for activity in branch["actions"]:
                    print(f"Case 114: Activity: {structure}, activity.is_end_activity: {structure.is_end_activity}")
                    if filter_finish_activities:
                        ...
                        if activity.is_end_activity and activity.is_finish_activity:
                            ...
                            if activity.is_end_activity:  # TODO:2023-12-08:  did not go here as "the process continues" is not end activity End Gateway
                                structure_contains_end_activity = True
                                need_end_gateway = False
                                end_id = "end_" + str(activity.id)
                                early_end_gateway = "(end) as end_" + str(activity.id)  # TODO:2023-12-09: (end) as end4
                ...
                if less_end_gateways2:
                    if structure.structure_needs_end_gateway == False:
                        need_end_gateway = False
                if need_end_gateway:  # TODO:2023-12-08:  here End Gateway
                ...
                if need_end_gateway == False and less_end_gateways2:
                    end_gateway = "activity_" + str(structure.id)
                    connections[connection_id] += "->" + end_gateway

            if less_end_gateways2 == True:
                if structure.structure_needs_end_gateway == True:
                    lanes[key].append("<> as " + end_gateway)  # TODO:2023-12-09: changed here
            connection_id += 1
            if less_end_gateways2:
                if structure.structure_needs_end_gateway == True:
        ...
        if structure.is_end_activity and structure.is_finish_activity:  # TODO:2023-12-09:  here End Gateway changed
            ...
            if connection_id < len(connections):
                ...
                connections[connection_id] += "->end"
        #elif structure.structure_needs_end_gateway == False:  # TODO:2023-12-09:  here End Gateway changed
            #if less_end_gateways2:
             #   last_gateway = "activity_" + str(structure.id)
...
def append_to_lane(key: str, lanes: {}, connection_id: int, connections: list, structure: Structure,
    ...
    if resolve_syntax_problems:
        ...
        if isinstance(structure, Activity):
            connection_name = "activity_" + str(structure.id)  # acitiviy_3
        else:
